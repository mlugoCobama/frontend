...
import { configEstadosTomaUnidades, configTablaTomaUnidades } from './modelos-toma-unidades';
...
@Component({
  ...
  templateUrl: './comisiones-toma-unidades.component.html',
...
export class ComisionesTomaUnidadesComponent implements OnInit{
   ...
   public data = [];
    ...
    public columnasVendedor: any[] = configTablaTomaUnidades;
    public misEstados= configEstadosTomaUnidades;
  ...
  /**  Despliega alerta de confirmación de eliminado de registro */
      confirmarDelete() {
        Swal.fire({
          ...
          preConfirm: () => {
            return this.tomaUnidadesService
        ...
        buscarDatos(datos){
          ...
          this.tomaUnidadesService.getLibroVentas(param.estado ,param.agencia, param.fechaInicial, param.fechaFinal, param.vendedor).subscribe(
            (response:any) => {
              if(response.status == 'success'){
                this.data = response.data;
                this.buscando = false;
          ...
          const response: any = await firstValueFrom(
            this.tomaUnidadesService.devolverPartida(row.id, { comentario: razon })
          );
        ...
        async avanzarEstado(row: any): Promise<void> {
          ...
          const response: any = await firstValueFrom(
            this.tomaUnidadesService.avanzarEstado(row.id)
          );
