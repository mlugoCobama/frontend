...
export class CapturaGasolineriasComponent implements OnInit {
  ...
  public catEmpresas: any; //Empresas (Gaseras)
  ...
  public lastIndex:  any;//ultimo index de la respuesta (fila del total)
  ...
  public getAllEmpresas() {
    this.catEmpresasService.getAll(2).subscribe(
      (response) => {
        if (response.success) {
          this.catEmpresas = response.data;
        } else {
  ...
  public saveInfo() {
    Swal.fire({
      ...
      if (result.value) {
        this.energeticosGasolineras.save(this.dataToSend).subscribe((response) => {
  ...
  public getInfoMes() {
    ...
    this.isLoad = false;
    this.showTable =true;
    this.energeticosGasolineras.getGasolinerias(this.mes, this.anio).subscribe((data) => {
      ...
      .then((text) => {
        ...
        const result = filasFiltradas.map((row) => {
          const object = this.headers.reduce((acc, header, index) => {
            if (header === "Planta") {
              const estacion = this.catEmpresas.find((e) => e.sucursal=== row[index]);
