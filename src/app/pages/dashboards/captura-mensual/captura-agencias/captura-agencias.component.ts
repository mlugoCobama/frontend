...
export class CapturaAgenciasComponent implements OnInit {
  ...
  private getMesAgencia() {
    ...
    this.agenciasService.getMesAgencias(this.mes, this.anio).subscribe(
      ...
      (error) => {
        this.alertService.alertError(
          `Error fetching data:, ${error}`, false);
      }
    );
  }
   ...
   * ------------------------------------------------------------------
   */
  private procesarCeldasCombinadas(
    filas: string[][]
  ...
  public save() {
    Swal.fire({
      ...
      if (result.value) {
        this.agenciasService.save(data).subscribe(
          (response) => {
  ...
  public update() {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.agenciasService.update(data).subscribe(
          (response) => {
            if (response.status === "success") {
              Swal.fire({
                title: "Guardado",
