...
@ViewChild('formDatosOC', { static: false }) formDatosOC!:  FormDatosEntregaOcComponent;
...
public proveedorSeleccionado: any;
  ...
  public generarOrden() {
    ...
    if (!this.formDatosOC.isValid()) {
      this.alertasService.mostrarAlerta( "Error", "Entrega y forma de pago son obligatorios", "warning","warning");
      this.sending = false;
      return;
    }
    ...
    this.ordenesComprasService.save(datos).subscribe(
      (response) => {
        if (response.status === "success") {
          this.alertasService.mostrarAlerta( "Guardado", "Se generó correctamente la orden de compra",
            "success", "success");
  ...
  public manejoCheck(prov: any): void {
    ...
    if (total === 0) {
      ...
      this.alertasService.mostrarAlerta("El total de la cotización debe ser mayor a 0",
        "Carga los precios y da click en el botón de guardar precios", "info", "info" );
      return;
    }
    this.proveedorSeleccionado = prov;
  ...
  totalCotizacion(prov){
    ...
    private enviarSolAutorizacion(){
      this.cotizacionesService.solicitarAutorizacion(this.solicitudCompra.id).subscribe(
       (response) => {
          if (response.status === "success") {
            this.alertasService.mostrarAlerta( "Enviado", "Se ha solicitado la autorización por parte de la planta",
              "success", "success");
            this.actualizarStatus.emit();
          } else {
